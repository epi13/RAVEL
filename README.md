...
>
> **MNCS-native reconstruction (2026-08, modernized 2026-09):** the reasoning core now also exists *in the MNCS language itself* under [`mncs/`](mncs/README.md) — twelve Source Profile 0.6/0.10 modules (status lattice with generic evidence envelopes, learning loop, checkpointed transactions, memory promotion with negative retention, snapshot-bound task contexts, knowledge lifecycle, Forge provider dispatch, refusable budgets, request/receipt binding, Digest32-bound content identities with lineage, provenance-carrying evidence envelopes), each verified end to end by language-owned experiments with layered body/SSA/backend agreement on all five executable backends (research bytecode, portable WASM, C11, LLVM, Cranelift). The Rust/Python/C trees are increasingly reference and compatibility surfaces for that reconstruction; the language-native tree is where new RAVEL semantics land first.
