...
>
> **MNCS-native reconstruction (2026-08):** the reasoning core now also exists *in the MNCS language itself* under [`mncs/`](mncs/README.md) — five Source Profile 0.5 modules (status lattice, learning loop, checkpointed transactions, memory promotion with negative retention, snapshot-bound task contexts), each verified end to end by language-owned experiments with layered body/SSA/backend agreement. The Rust/Python/C trees are increasingly reference and compatibility surfaces for that reconstruction; the language-native tree is where new RAVEL semantics land first.
