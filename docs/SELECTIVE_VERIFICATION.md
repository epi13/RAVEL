...
For MNCS source paths, `mncs impact` returns semantic impact and a compact
compiler-issued `source_subject` binding (artifact identity, module, profile,
semantic identity, and fingerprint) from one front-end session. Direct
source-test selection requests `--include-test-inventory` so the compiler's
recognized test cases remain available from that same session. When RAVEL
plans against a repository-owned external verification-obligation inventory,
it uses `source_subject` for evidence binding and does not request source test
inventory or launch a second cold source compilation. The repository
inventory supplies the external verification selection; source test inventory
remains separate evidence and is not required for that plan.
